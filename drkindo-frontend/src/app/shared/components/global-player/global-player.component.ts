    ...
    @if (player.playing) {
      <div class="mini-player">
        ...
        <div class="mini-content">
          ...
          <div class="mini-info">
            <div class="mini-title" style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
              <span style="overflow: hidden; text-overflow: ellipsis;">{{ player.playing.filename }}</span>
              <button class="mini-btn" (click)="goToFolder()" title="Voir dans le dossier" style="padding: 2px;">
                ...
                <div class="speed-menu">
                  @for (rate of playbackRates; track rate) {
                    <button
                      ...
                      (click)="player.changePlaybackRate(rate); showSpeedMenu = false"
                    >
                      {{ rate }}x
                    </button>
                  }
...
export class GlobalPlayerComponent {
  ...
  constructor(public player: AudioPlayerService, private router: Router) {}

  goToFolder() {
    if (this.player.playing?.folder?.id) {
      this.router.navigate(['/explorer'], { 
        queryParams: { folderId: this.player.playing.folder.id, highlight: this.player.playing.id }
      });
    } else {
      this.router.navigate(['/explorer']);
    }
